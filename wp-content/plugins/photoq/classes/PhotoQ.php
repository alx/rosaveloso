...
class PhotoQ extends PhotoQObject
{
	...
	function options_page()
	{
		...
		if (isset($_POST['showWatermarkUploadPanel'])) {
			check_admin_referer('photoq-updateOptions','photoqUpdateOptionsNonce');
			...
			if(!empty($changedSizes) || $this->_oc->hasChanged(array('contentView','excerptView','exifTags','exifDisplay','originalFolder'))){
					
				/*$this->rebuildPublished($changedSizes, $this->_oc->hasChanged('exifTags'),
				...
				PhotoQHelper::debug('update exif: ' .$updateExif.' update content:' . $updateContent .' update original folder: '. $updateOriginalFolder);
				...
				if($publishedPhotoIDs = $this->_db->getAllPublishedPhotoIDs()){
					...
					$updateContent, $updateOriginalFolder,
	...
	function manage_page()
	{
		...
		if ( isset($_POST['add_entry']) || isset($_POST['ftp_upload']) ) {
			...
			$this->uploadPhoto($_FILES['Filedata']['name'], '', '', '');
			...
			if (isset($_POST['save_batch'])) {
				...
				for ($i = 0; $i<$no_upl; $i++) {
					$this->update_queue(attribute_escape($_POST['img_id'][$i]), attribute_escape($_POST['img_title'][$i]), $_POST['img_descr'][$i], attribute_escape($_POST['tags_input'][$i]), attribute_escape($_POST['img_slug'][$i]), attribute_escape($_POST['img_author'][$i]), attribute_escape($_POST['img_position'][$i]), attribute_escape($_POST['img_position'][$i]), attribute_escape($_POST['img_parent'][$i]), $qLength, $i);
				}
			...
			if (isset($_POST['update_queue'])) {
				...
				$this->update_queue(attribute_escape($_POST['img_id']), attribute_escape($_POST['img_title']), $_POST['img_descr'], attribute_escape($_POST['tags_input']), attribute_escape($_POST['img_slug']), attribute_escape($_POST['img_author']), attribute_escape($_POST['img_position']), attribute_escape($_POST['img_old_position']),attribute_escape($_POST['img_parent'][0]), attribute_escape($_POST['q_length']));
			}
	 ...
	 * @return unknown_type
	 */
	function batchRebuildPublished($ids, $changedSizes, $updateExif, $updateContent,
		...
		if($updateOriginalFolder)
			$this->_errStack->pushCallback(array('PhotoQErrorHandler', 'silencePhotoNotFoundErrors'));
		
		//rebuild it
		$photo = &$this->_db->getPublishedPhoto($currentId);
		
		//PhotoQHelper::debug('photo: ' . print_r($photo,true));
		//re-enable all errors
		//if($updateOriginalFolder)
		//	$this->_errStack->popCallback();
		
		if($photo)
			$photo->rebuild($changedSizes, $updateExif, $updateContent,
